...
uint8_t rxData[16U];
static const uint8_t txData[] = {'H','E','L','L','O',' ','M','A',
                                  'Y','A','!','!', 0, 0, 0, 0};
...
int main(void)
{
  ...
  printf("Board ready, waiting for CAN messages...\r\n");
  ...
  while (1)
  {
    ...
    /* Check ID only, print whatever we received */
        ...
        (rxHeader.DataLength == FDCAN_DLC_BYTES_16))
    {
      ...
      printf("Message received! ID: 0x%03lX | Data: ", rxHeader.Identifier);
      for (int i = 0; i < 16; i++) {
          printf("%c", rxData[i]);
      }
      printf("\r\n");
