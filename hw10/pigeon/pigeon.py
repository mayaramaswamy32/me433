...
using a breadcrumb lure you move with the arrow keys.
Don't let them escape or land on the WRONG target or you lose a life!
...
Streak bonus: land 3 pigeons correctly IN A ROW → earn +1 life.
Any mistake (wrong target or escaped pigeon) resets the streak to 0.

Architecture note
-----------------
get_player_input() is the ONLY place that reads the keyboard.
Swap it out for an API call / AI agent whenever you're ready.
"""
...
LURE_SPD       = 5
MAX_BIRDS      = 6
SPAWN_INT      = 140   # frames between spawns
NUM_COLORS     = 4     # start with 4 colours; easy to extend
STARTING_LIVES = 27     # ← change this to give yourself more lives
STREAK_TARGET  = 3     # correct landings in a row needed to earn a life
...
def get_player_input() -> tuple[int, int]:
    """
    Returns (dx, dy) indicating how to move the lure this frame.
    Values are in {-1, 0, 1} for each axis.

    TODO: replace with an API / AI call that returns the same shape,
...
def new_game():
    ...
    state = dict(
        ...
        over    = False,
        stimer  = 60,           # start a little early
        wind    = [0.0, 0.0],
        wtimer  = 0,
        targets = make_targets(),
        msg     = "",           # short on-screen feedback
        msg_ttl = 0,
        streak  = 0,            # sequential correct landings counter
        paused  = False,        # X toggles the pause/quit screen
        prev_x  = False,        # previous frame's X key state (for edge detection)
        prev_y  = False,        # same for Y
        prev_q  = False,        # same for Q
    )
    for _ in range(2):          # start with two birds already in play
        add_bird()
...
def add_bird():
    ...
    s['birds'].append(dict(
        x    = float(x),  y    = float(y),
        vx   = vx,        vy   = vy,
        ti   = random.randint(0, NUM_COLORS - 1),  # target index
        chaos= random.uniform(0.22, 0.68),
        pull = random.uniform(0.06, 0.20),          # how strongly it chases the lure
        flap = 0,         up   = True,
...
def hurt():
    """Lose a life and reset the streak to zero."""
    state['streak']  = 0
    state['lives']  -= 1
    if state['lives'] <= 0:
        state['over'] = True
...
def update():
    ...
    # X toggles the pause/quit screen — only on the rising edge (key just pressed)
    x_down = keyboard.x
            ...
            if math.hypot(b['x'] - t['x'], b['y'] - t['y']) < t['r']:
                if b['ti'] == s['targets'].index(t):
                    ...
                    if s['streak'] >= STREAK_TARGET:
                        # streak bonus — earn a life!
                        s['lives']  += 1
                        s['streak']  = 0
                        flash_msg(f"+1 LIFE!  3-streak bonus!  Lives: {s['lives']}", 90)
                    else:
                        flash_msg(
                            f"+10  Streak: {s['streak']}/{STREAK_TARGET}"
                            f"  (+1 life at {STREAK_TARGET}!)", 55
                        )
                else:
                    # ✗ wrong target → disaster splat + reset streak
...
def draw_bird(b):
    x  = int(b['x']);  y = int(b['y'])
    c  = PALETTE[b['ti']]
    wo = -10 if b['up'] else 8   # wing vertical offset
    ...
    # colour badge (shows which target this pigeon belongs to)
    screen.draw.filled_circle((x, y), 7, c)
    screen.draw.circle((x, y), 7, WHITE)
    screen.draw.text(PAL_LABEL[b['ti']], center=(x, y-1),
...
def draw_streak_pips(streak, x, y):
    """Small filled/empty circles showing progress toward the streak bonus."""
    for i in range(STREAK_TARGET):
        cx = x + i * 22
        if i < streak:
            screen.draw.filled_circle((cx, y), 8, T_GREEN)
        else:
            screen.draw.filled_circle((cx, y), 8, DK_GRAY)
...
def draw():
    ...
    # poop splats (drawn before targets so targets stay on top)
    for sp in s['splats']:
    ...
    for t in s['targets']:
        ...
        screen.draw.text(t['ltr'], center=(t['x'], t['y'] - 1),
    ...
    # pigeons (drawn on top of lure so they're always visible)
    for b in s['birds']:
        draw_bird(b)

    # ── HUD ──
    screen.draw.text(f"Score: {s['score']}", topleft=(12, 10),
                     color=BLACK, fontsize=32)
    screen.draw.text(f"Lives: {s['lives']}", topleft=(12, 48),
                     color=DARK_RED, fontsize=28)

    # streak pips + label
    screen.draw.text("Streak:", topleft=(12, 84), color=DK_GRAY, fontsize=22)
    draw_streak_pips(s['streak'], 105, 93)
    screen.draw.text("(3 in a row = +1 life)", topleft=(178, 84),
                     color=DK_GRAY, fontsize=18)

    # wind indicator
    wx = s['wind'][0]
    wind_label = ("<-- Wind" if wx < -0.2
                  else "Wind -->" if wx > 0.2
                  else "Wind: calm")
    screen.draw.text(wind_label, topright=(WIDTH - 14, 10),
                     color=DK_GRAY, fontsize=22)

    # controls hint at bottom
    screen.draw.text(
        "Arrow keys: move lure  |  Match pigeon badge to target  |  X: pause",
        center=(WIDTH // 2, HEIGHT - 14),
        color=DK_GRAY, fontsize=17)
    ...
    # ── pause overlay ──
    if s['paused']:
        # dim the whole screen with a dark translucent panel
        ...
        # pigeon icon (just a little drawing)
        ...
        # text
        screen.draw.text("PAUSED", center=(cx, cy - 20),
                         color=T_YELLOW, fontsize=46)
        # Y button
        screen.draw.filled_rect(Rect(cx - 180, cy + 40, 160, 52), T_GREEN)
        screen.draw.rect(Rect(cx - 180, cy + 40, 160, 52), WHITE)
        screen.draw.text("Y  Continue", center=(cx - 100, cy + 66),
        ...
        screen.draw.text("Q  Quit", center=(cx + 100, cy + 66),
                         color=WHITE, fontsize=26)

    # ── game-over overlay ──
    if s['over']:
        screen.draw.filled_rect(Rect(WIDTH//2 - 250, HEIGHT//2 - 110, 500, 220),
                                (10, 10, 10))
